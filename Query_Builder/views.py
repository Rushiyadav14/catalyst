    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if file.name.endswith('.csv'):
                required_columns = [
                    'name', 'domain', 'year founded', 'industry', 'size range',
                ...
                if not all(column in reader.fieldnames for column in required_columns):
                    return HttpResponse('<script>alert("Invalid file format. invalid Excel column"); window.location.href = "/upload";</script>')
                else:
                    process_csv(file)
                ...
                # Check if all required columns are present in the uploaded file
                if not all(column in header_row for column in required_columns):
                    return HttpResponse('<script>alert("Invalid file format. invalid Excel column"); window.location.href = "/upload";</script>')
                else:
                  process_xlsx(sheet,file)

            return render(request, 'filter.html')
        else:
            return HttpResponse('<script>alert("Invalid file format. Please upload a .csv or .xlsx file."); window.location.href = "/upload";</script>')
    else:
        form = UploadFileForm()
